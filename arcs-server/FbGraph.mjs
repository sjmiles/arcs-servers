...
export const FbGraph = Firedb => {
  ...
  const ChildField = class extends Eventer {
    ...
    _createField(fieldName, fieldSchema) {
      ...
      const field = new ChildField(this, fieldPath, fieldName, fieldSchema, event => this._onEvent(event));
      field.$fromJoin = fromJoin;
      return field;
    }
  };

  const Field = class extends ChildField {
    constructor(path, schema, onevent) {
      super(null, path, path.split('/').pop(), schema, onevent);
    }
  };
